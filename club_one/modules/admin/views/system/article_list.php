...
<div class="headbar">
	<div class="operating">
		<?php if ($self->auth_rights('admin/'.$self->request->directory().'/main/add/'.$self->request->param('param')) !== FALSE ) { ?>
		<a href="<?php echo URL::site('admin/'.$self->request->directory().'/main/add/'.$self->request->param('param')) ?>">
		...
		<a href="javascript:;">
		<button class="operating_btn" type="button" onclick="selectWindow('/admin/cms/main/select_list?catalog_id=<?php echo $catalog_id; ?>', 'select_article_url', '/admin/cms/main/selected')"><span class="addition"><?php echo I18n::get('btn_associate_other', 'catalog') ?></span></button></a>
		<?php } ?>
		...
		<a href="<?php echo URL::site('admin/system/catalog') ?>"><button class="operating_btn" type="button"><span class="return"><?php echo I18n::get('btn_return', 'common') ?></span></button></a>
	</div>
	<div class="field">
		...
		<table id="list_table" class="list_table">
			...
			<tbody id="list_table" class="list_table">
				<?php foreach ($data as $v) { ?>
                    <tr class="gradeA">
                    	<td><input type='checkbox' name='<?php echo $pk ?>[]' value='<?php echo $v->link_id ?>' class="pk_checkbox" /></td>
                   		<?php foreach ($columns as $k => $col) { ?>
                    	<td><?=$self->$col['func']($v, $k)?></td>
                    	<?php } ?>
                   	 	<td><?=$self->handle($v->$pk, $v)?></td>
                    </tr>
                <?php } ?>
			</tbody>			
		</table>
