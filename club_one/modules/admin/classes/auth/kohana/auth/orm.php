...
class Kohana_Auth_ORM extends Auth {
	...
	public function get_role($language_id = 0)
	{
		...
		if ( ! $role)
		{
			$role = ORM::factory('auth_admin_roles')->where('id', '=', $this->get_user()->role_id)->where('status', '=', 1)->find();			
			$role = $this->bind_language('auth_admin_roles', $role, $language_id);	
	...
	public function get_rights($language_id = 0)
	{
		...
		if ( ! $rights)
		{
			$role = ORM::factory('auth_admin_roles')->where('id', '=', $this->get_user()->role_id)->find();
			if ($role->super_admin == 1)
			{
				$rights = ORM::factory('auth_admin_rights')->where('status', '=', 1)->find_all()->as_array('id');
			}
			elseif (trim($role->rights) != '')
			{
				$rights = ORM::factory('auth_admin_rights')->where('id', 'in', explode(',', $role->rights))->where('status', '=', 1)->find_all()->as_array('id');
			}
			$rights = $this->bind_language('auth_admin_rights', $rights, $language_id);
			$data = array();
