...
class Includes_Common{ 
	...
	public function sub($id, $status = NULL, $args = array())
	{
		...
		{
			$data = $this->bind_language($this->_model_name, $data, self::language_id());
		}
		return $data;
	}
	/**
	...
	{
		$cache_id = 'TREEVIEWS_DATA_'.strtoupper($this->_model_name).$parent_id.self::language_id().md5(implode(',', $columns)).$status;
...
// 			print_r($v);
	...
	public function get_options($key = 'pk', $column = 'name')
	{
		$data = $this->get_treeviews_data(0);
