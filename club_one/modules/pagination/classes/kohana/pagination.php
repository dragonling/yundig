...
class Kohana_Pagination {
	...
	public function url($page = 1)
	{
		...
		switch ($this->config['current_page']['source'])
		{
			...
			case 'route':
				...
				$params = $request->route()->matches($request->uri());
				$params[$this->config['current_page']['key']] = $page;
				
				if (isset($_GET['language_id']))
				{
					$lang = $_GET['language_id'];
					unset($_GET['language_id']);
					$query = URL::site($request->route()->uri($params)).URL::query();
					$_GET['language_id'] = $lang;
				}
				else
				{
					$query = URL::site($request->route()->uri($params)).URL::query();
	...
	public function render($view = NULL)
	{
		...
		{
			// Load the view file
			$view = View::factory()->set_filename($view);
		}
